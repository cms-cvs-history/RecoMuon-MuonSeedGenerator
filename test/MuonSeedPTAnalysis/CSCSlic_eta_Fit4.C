void CSCSlic_eta_Fit4(int s1, int s2, double f1, double f2){
...
TFile *file = TFile::Open("seedN_5-200DTCSC_2.root");
FILE *dfile = fopen("ME_Data_2Seg_N.txt","a");

TString suffixps = ".jpg";

char dphi_case[22];
sprintf(dphi_case,"ME_%d-%d_heta_rdphiPtA",s1,s2);
TString dphi_type = dphi_case;

char dphi_case1[20];
sprintf(dphi_case1,"ME_%d-%d_heta_rdphiA",s1,s2);
TString dphi_type1 = dphi_case1;
char deta_case1[20];
sprintf(deta_case1,"ME_%d-%d_heta_rdetaA",s1,s2);
...
; 
 ...
 eta_dphiPt_prf1->SetTitle("eta_dphiPt_profile  eta 1.0 ~ 2.4");
 ...
 for (int i=1; i<76; i++) {
     ...
     if ((r > f2)||(r < f1)) {
        pt_rt1[i-1]=0.0;
         ...
         //double sigma_etaf = (perr1*perr1) + (perr0*perr0*xbin1a[i]*xbin1a[i]);
 ...
 eta_dphi_prf1->SetTitle("eta_dphi_profile  eta 1.0 ~ 2.4");
 ...
 pt_accuracy->SetTitle("pT accuracy  eta 1.0 ~ 2.4");
