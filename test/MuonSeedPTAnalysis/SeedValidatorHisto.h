...
public:
 ...
 H2DRecHit1(std::string name_) {
    ...
    heta_NSeed  = new TH2F(N1+"_heta_NSeed", " eta vs N of Seed", 59, -2.95, 2.95, 20, -0.25, 9.75);
    ...
    hSeedPhi0 = new TH1F(N1+"_hSeedPhi0"," seed phi distribution ",640, -3.1415, 3.1415);
    hStaPhi0  = new TH1F(N1+"_hStaPhi0"," sta phi distribution ",640, -3.1415, 3.1415);
    hSeedPhi1 = new TH1F(N1+"_hSeedPhi1"," seed phi distribution ",640, -3.1415, 3.1415);
    hStaPhi1  = new TH1F(N1+"_hStaPhi1"," sta phi distribution ",640, -3.1415, 3.1415);
    ...
    h_dh_st1 = new TH2F(N1+"_h_dh_st1", " dEta from vector vs station bad pt", 19, -4.75, 4.75,400,-1.0,1.0);
    h_df_st1 = new TH2F(N1+"_h_df_st1", " dPhi from vector vs station bad pt", 19, -4.75, 4.75,800,-0.1,0.1);
    h_dx_st1 = new TH2F(N1+"_h_dx_st1", " dx from position vs station bad pt", 19, -4.75, 4.75,400,-4.0,4.0);
    h_dy_st1 = new TH2F(N1+"_h_dy_st1", " dy from position vs station bad pt", 19, -4.75, 4.75,400,-4.0,4.0);

    //  pt vs. nHits and chi2
    h_pt_nHits = new TH2F(N1+"_h_pt_nHits", " pt vs nHits ", 104, -0.25, 51.75, 250, 0.0,250.0);
    h_pt_chi2  = new TH2F(N1+"_h_pt_chi2" , " pt vs chi2  ", 500, 0., 50., 250, 0.0,250.0);

    h_pt_nHits_e = new TH2F(N1+"_h_pt_nHits_e", " pt vs nHits ", 104, -0.25, 51.75, 250, 0.0,250.0);
    h_pt_chi2_e  = new TH2F(N1+"_h_pt_chi2_e" , " pt vs chi2  ", 500, 0., 50., 250, 0.0,250.0);

    h_Nsta_Nseed = new TH2F(N1+"_h_Nsta_Nseed", " N_STA vs N_Seed ", 30, -0.25, 14.75, 30, -0.25, 14.75);
    h_RecoSim    = new TH1F(N1+"_h_RecoSim ", " N_recoMuon - N_simMuon ", 40, -5.25, 14.75);
 ...
 H2DRecHit1(TString name_, TFile* file) {
    ...
    hSeedPhi0 = (TH1F *) file->Get(name+"_hSeedPhi0");
    hStaPhi0  = (TH1F *) file->Get(name+"_hStaPhi0");
    hSeedPhi1 = (TH1F *) file->Get(name+"_hSeedPhi1");
    hStaPhi1  = (TH1F *) file->Get(name+"_hStaPhi1");
    ...
    h_dh_st1 = (TH2F *) file->Get(name+"_h_dh_st1");
    h_df_st1 = (TH2F *) file->Get(name+"_h_df_st1");
    h_dx_st1 = (TH2F *) file->Get(name+"_h_dx_st1");
    h_dy_st1 = (TH2F *) file->Get(name+"_h_dy_st1");

    h_pt_nHits = (TH2F *) file->Get(name+"_h_pt_nHits");
    h_pt_chi2  = (TH2F *) file->Get(name+"_h_pt_chi2");

    h_pt_nHits_e = (TH2F *) file->Get(name+"_h_pt_nHits_e");
    h_pt_chi2_e  = (TH2F *) file->Get(name+"_h_pt_chi2_e");

    h_Nsta_Nseed = (TH2F *) file->Get(name+"_h_Nsta_Nseed");
    h_RecoSim    = (TH1F *) file->Get(name+"_h_RecoSim");
 ...
 void Fill1(int csc_nu,int dt_nu,int all_nu, int sim_nu, double eta_simtrk) {
 ...
 void Fill1j(double eta_sta, double sta_qbp, double sta_qbpt, double sta_pt, double sta_pa) {
 ...
 void Fill1k(int station, double dh_v, double df_v, double dx_p, double dy_p) {
      h_dh_st1->Fill(station, dh_v);    
      h_df_st1->Fill(station, df_v);    
      h_dx_st1->Fill(station, dx_p);    
      h_dy_st1->Fill(station, dy_p);    
 }
 void Fill1l0(double seed_phi, double sta_phi) {
      hSeedPhi0->Fill(seed_phi);
      hStaPhi0->Fill(sta_phi);
 }
 void Fill1l1(double seed_phi, double sta_phi) {
      hSeedPhi1->Fill(seed_phi);
      hStaPhi1->Fill(sta_phi);
 }

 void Fill1m(double pt, int nhits, double chi2) {
      h_pt_nHits->Fill(nhits, pt);
      h_pt_chi2->Fill(chi2, pt);
 }

 void Fill1n(double pt, int nhits, double chi2) {
      h_pt_nHits_e->Fill(nhits, pt);
      h_pt_chi2_e->Fill(chi2, pt);
 }
 
 void Fill1o(int Nsta, int Nseed, int reco_sim) {
      h_Nsta_Nseed->Fill(Nseed, Nsta);
      h_RecoSim->Fill(reco_sim);
 }
...
public:
 ...
 H2DRecHit2(std::string name_) {
    ...
    heta_cscrh = new TH2F(N2+"_heta_cscrh", " csc rechits vs eta ", 250, 0.0, 2.5, 20, -0.25, 9.75);
    heta_dtrh  = new TH2F(N2+"_heta_dtrh",  " dt  rechits vs eta ", 250, 0.0, 2.5, 20, -0.25, 9.75);
...
public:
 ...
 H2DRecHit3(std::string name_) {
    ...
    hsimEta_NSeg  = new TH2F(N3+"_hsimEta_NSeg", "No sta, sim_eta vs NSeg", 59, -2.95, 2.95, 20, -0.25, 9.75);
 ...
 H2DRecHit3(TString name_, TFile* file) {
    ...
    hsimEta_NSeg    = (TH2F *) file->Get(name+"_hsimEta_NSeg");
  ...
  void Fill3b(double simEta, int NSeg, int NGoodSeg) {
       hsimEta_NSeg->Fill(simEta, NSeg);
