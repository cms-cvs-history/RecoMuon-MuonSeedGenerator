void DTSlic_eta_Fit(int s1, int s2, double f1, double f2){
...
TFile *file = TFile::Open("seed_5-200DTCSC_2.root");
FILE *dfile = fopen("MB_Data_2Seg_P.txt","a");

TString suffixps = ".jpg";

char dphi_case[22];
sprintf(dphi_case,"MB_%d-%d_heta_rdphiPtA",s1,s2);
TString dphi_type = dphi_case;

char dphi_case1[20];
sprintf(dphi_case1,"MB_%d-%d_heta_rdphiA",s1,s2);
TString dphi_type1 = dphi_case1;
char deta_case1[20];
sprintf(deta_case1,"MB_%d-%d_heta_rdetaA",s1,s2);
...
; 
 ...
 eta_dphiPt_prf1->SetTitle("eta_dphiPt_profile  eta 0.0 ~ 1.1");
 ...
 for (int i=1; i<56; i++) {
     ...
     /*double L3 = pr1 - nsigmas * pr2;
     double H3 = pr1 + nsigmas * pr2;
   
     gaus->SetParLimits(1,0.,1.);
    
     heta_dphi_pjy->Fit("gaus","N0R","",L3,H3);
     pr0 = gaus->GetParameter(0);
