void CSCSlic_eta_Fit5(int s1, int s2, double f1, double f2){
...
FILE *dfile = fopen("SME_Data.txt","a");

TString suffixps = ".jpg";

char dphi_case[20];
sprintf(dphi_case,"SME_%d%d_heta_rdphiPt1",s1,s2);
TString dphi_type = dphi_case;

char dphi_case1[16];
sprintf(dphi_case1,"SME_%d%d_heta_phir",s1,s2);
TString dphi_type1 = dphi_case1;

char dphi_case2[18];
sprintf(dphi_case2,"SME_%d%d_heta_rdphi1",s1,s2);
TString dphi_type2 = dphi_case2;

char deta_case1[16];
sprintf(deta_case1,"SME_%d%d_heta_etar",s1,s2);
...
// ********************************************************************
    ...
    heta_dphi    = (TH2F *) file->Get(dphi_type2);
    heta_phir    = (TH2F *) file->Get(dphi_type1);
...
; 
 ...
 double prf1[75]={0.0};
 double xbin1[75]={0.0};
 double prfErrY1[75]={0.0};
 double prfErrX1[75]={0.0};
 for (int i=0; i<75; i++) {
     prfErrX1[i]=0.01;
 }
 ...
 eta_dphiPt_prf1 = new TGraphErrors(75,xbin1,prf1,prfErrX1,prfErrY1);
 ...
 double prf1a[75]={0.0};
 double xbin1a[75]={0.0};
 double prfErrY1a[75]={0.0};
 double prfErrX1a[75]={0.0};
 ...
 for (int i=1; i<76; i++) {
     ...
     heta_phir->ProjectionY("heta_phir_pjy",j,j+1);
     ...
     heta_dphi_pjy->Fit("gaus","N0");
     double pr1 = gaus->GetParameter(1);
     double pr2 = gaus->GetParameter(2);
     double L1 = pr1 - nsigmas * pr2;
     double H1 = pr1 + nsigmas * pr2;
     heta_dphi_pjy->Fit("gaus","N0R","",L1,H1);
     pr1 = gaus->GetParameter(1);
     pr2 = gaus->GetParameter(2);
 
     /*
     heta_phir_pjy->Fit("gaus","N0");
     double pt1 = gaus->GetParameter(1);
     double pt2 = gaus->GetParameter(2);
 ...
 eta_dphi_prf1 = new TGraphErrors(75,xbin1a,prf1a,prfErrX1a,prfErrY1a);
 eta_dphi_prf1->SetTitle("eta_dphi_profile");
 ...
 pt_accuracy = new TGraph(75,xbin1a,pt_rt1);
 pt_accuracy->SetTitle("pT accuracy  ");
